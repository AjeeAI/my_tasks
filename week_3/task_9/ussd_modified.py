...
while True:
    ...
    if ussdInput == "1":
            ...
            while attempts > 0:
                ...
                if len(entry) == 10 and entry.isdigit():
                    ...
                    if amount.isdigit() and int(amount) <= balance:
                        balance -= int(amount)
                        print(f"Transfer of ₦{amount} to account {entry} successful.")
                    ...
                    if 1 <= entry <= len(banks):
                        ...
                        while acc_attempts > 0:
                            ...
                            if len(acc_entry) == 10 and acc_entry.isdigit():
                                ...
                                if amount.isdigit() and int(amount) <= balance:
                                    balance -= int(amount)
                                    print(f"Transfer of ₦{amount} to {banks[entry-1]} account {acc_entry} successful.")
                                    print(f"Remaining balance: ₦{balance:,}")
                                    acc_attempts = 0
                                    attempts = 0
                                    break
        ...
        if airtimeInput == "1":
            ...
            if amount.isdigit() and int(amount) <= balance:
                balance -= int(amount)
                print(f"Airtime of ₦{amount} bought successfully for yourself.")
                print(f"Remaining balance: ₦{balance:,}")
            else:
                print("Insufficient funds or invalid amount!")
            ...
            while attempts > 0:
                ...
                if phoneNumber.isdigit() and len(phoneNumber) == 11:
                    ...
                    if amount.isdigit() and int(amount) <= balance:
                        balance -= int(amount)
                        print(f"Airtime of ₦{amount} bought successfully for {phoneNumber}.")
                        print(f"Remaining balance: ₦{balance:,}")
                        break
